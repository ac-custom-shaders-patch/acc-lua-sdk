...

{
  ...
  __mul = function(q, r)
    ...
    else error('quats can only be multiplied by quats and vec3s', 2) end
  end,
  ...
  __index = {
    ...
    isquat = function(x) return ffi.istype(ctquat, x) end,
    tmp = function() return qQtmp2 end,
    ...
    setBetween = function(q, u, v)
      ...
      elseif dot < -.99999 then
        vQtmp1.x, vQtmp1.y, vQtmp1.z = 1, 0, 0
        vQtmp1:cross(u)
        if #vQtmp1 < .00001 then
          vQtmp1.x, vQtmp1.y, vQtmp1.z = 0, 1, 0
          vQtmp1:cross(u)
        end
        vQtmp1:normalize()
        return q:setAngleAxis(math.pi, vQtmp1)
      end
    ...
    setDirection = function(q, x, y, z)
      if vec3.isvec3(x) then x, y, z = x.x, x.y, x.z end
      vQtmp2.x, vQtmp2.y, vQtmp2.z = x, y, z
      return q:setBetween(tQfwd, vQtmp2)
    end,
    ...
    slerp = function(q, r, t, out)
      ...
      r:scale(math.sin(t * theta), qQtmp1)
      return out:add(qQtmp1):scale(1 / math.sin(theta))
    end
  }
}
