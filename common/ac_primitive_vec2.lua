...
return {
  ...
  type = {
    ...
    __mul = function(v, u)
      ...
      else error('vec2s can only be multiplied by vec2s and numbers', 2) end
    end,
    __div = function(v, u)
      if type(v) == 'number' then return vec2(v, v) / u end 
      if vec2.isvec2(u) then return v:div(u, vec2())
      elseif type(u) == 'number' then return v:scale(1 / u, vec2())
      else error('vec2s can only be divided by vec2s and numbers', 2) end
    end,
    __pow = function(v, u)
      if type(v) == 'number' then return vec2(v, v) ^ u end 
      if vec2.isvec2(u) then return v:pow(u, vec2())
      elseif type(u) == 'number' then return vec2(v.x ^ u, v.y ^ u)
      else error('vec2s can only be raised to power of vec2s and numbers', 2) end
    end,
