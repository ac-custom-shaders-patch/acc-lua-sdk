...
return {
  ...
  type = {
    ...
    __mul = function(v, u)
      ...
      else error('vec4s can only be multiplied by vec4s and numbers', 2) end
    end,
    __div = function(v, u)
      if type(v) == 'number' then return vec4(v, v, v, v) / u end 
      if vec4.isvec4(u) then return v:div(u, vec4())
      elseif type(u) == 'number' then return v:scale(1 / u, vec4())
      else error('vec4s can only be divided by vec4s and numbers', 2) end
    end,
    __pow = function(v, u)
      if type(v) == 'number' then return vec4(v, v, v, v) ^ u end 
      if vec4.isvec4(u) then return v:pow(u, vec4())
      elseif type(u) == 'number' then return vec4(v.x ^ u, v.y ^ u, v.z ^ u, v.w ^ u)
      else error('vec4s can only be raised to power of vec4s and numbers', 2) end
    end,
