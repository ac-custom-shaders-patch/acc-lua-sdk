...

{
  ...
  __eq = function(v, o) if rawequal(o, nil) or rawequal(v, nil) then return rawequal(v, o) end return ffi.istype(ctrgbm, v) and ffi.istype(ctrgbm, o) and v.rgb == o.rgb and v.mult == o.mult end,
  ...
  __index = {
    ...
    colors = {},
    isrgbm = function(r) return ffi.istype(ctrgbm, r) end,
    tmp = function() return vMtmp1 end,
...
vMtmp1 = rgbm()
rgbm.colors.transparent = rgbm(0, 0, 0, 0)
rgbm.colors.black = rgbm(0, 0, 0, 1)
rgbm.colors.silver = rgbm(0.75, 0.75, 0.75, 1)
rgbm.colors.gray = rgbm(0.5, 0.5, 0.5, 1)
rgbm.colors.white = rgbm(1, 1, 1, 1)
rgbm.colors.maroon = rgbm(0.5, 0, 0, 1)
rgbm.colors.red = rgbm(1, 0, 0, 1)
rgbm.colors.purple = rgbm(0.5, 0, 0.5, 1)
rgbm.colors.fuchsia = rgbm(1, 0, 1, 1)
rgbm.colors.green = rgbm(0, 0.5, 0, 1)
rgbm.colors.lime = rgbm(0, 1, 0, 1)
rgbm.colors.olive = rgbm(0.5, 0.5, 0, 1)
rgbm.colors.yellow = rgbm(1, 1, 0, 1)
rgbm.colors.orange = rgbm(1, 0.5, 0, 1)
rgbm.colors.navy = rgbm(0, 0, 0.5, 1)
rgbm.colors.blue = rgbm(0, 0, 1, 1)
rgbm.colors.teal = rgbm(0, 0.5, 0.5, 1)
rgbm.colors.cyan = rgbm(0, 0.5, 1, 1)
rgbm.colors.aqua = rgbm(0, 1, 1, 1)
