...
function ac.grabCamera(reason)
  local r = ffi.C.lj_grabbedcamera_new__game(reason ~= nil and tostring(reason) or nil)
  if r == nil then return nil, __util.strref(ffi.C.lj_grabbedcamera_lasterror__game()) end
  return ffi.gc(r, ffi.C.lj_grabbedcamera_gc__game) 
end
...
ffi.metatype('grabbedcamera', { __index = {
  ...
  ---@return boolean
  active = ffi.C.lj_grabbedcamera_active__game,
  ---Releases held camera and allows Assetto Corsa to control camera as usual.
  dispose = ffi.C.lj_grabbedcamera_dispose__game,
  ...
  normalize = ffi.C.lj_grabbedcamera_normalize__game,
} })
