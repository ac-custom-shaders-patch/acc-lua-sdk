...
local MallocAllocatorT__mt = {
  __index = {
    ...
    deallocate = function(self, p)
      if p ~= 0 then ffi.C.lj_free2(p, 'v') end
    end,
...
function Vector:remove(i)
  ...
  if self._size - i > 0 then
    ffi.C.lj_memmove(self.raw, self.raw + (i - 1), self.raw + i, (self._size - i) * self.__ctSize)
  end
  self._size = self._size - 1
  if self.__keepAlive then
...
return function(ct, cb, keepAlive)
  if type(ct) ~= 'cdata' then error('argument 1 is not a valid “cdata”', 2) end
